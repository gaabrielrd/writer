...
- Implementação da feature `src/features/editor`:
  - `model/editorState.ts`: Estrutura de conteúdo rico do capítulo, status de salvamento (`saved`, `saving`, `unsaved`, `error`), e lista de marcações de lore ativas.
  ...
  - `components/EditorPage.tsx`: Tela de escrita do capítulo (`/books/:bookId/editor/:chapterId`) contendo cabeçalho com status de salvamento, contador de palavras, navegador sequencial de capítulos e área de escrita.
  - `components/RichEditor.tsx`: Componente de edição rica (títulos, negrito, itálico, citações, diálogos) com marcação visual sutil sobre termos casados pelo `loreMatcher`, alternância entre modo de escrita e modo leitura com lore, e atalhos de teclado.
  - `components/MentionMenu.tsx`: Menu suspenso flutuante acionado ao digitar `@` com busca instantânea de entidades, navegação por teclado e inserção direta no texto.
  ...
  - `hooks/useAutoSave.ts`: Hook para salvamento automático com debounce de 1 segundo e indicação visual de status.
  - `tests/`: Testes de unidade e componentes cobrindo digitação, atalhos, salvamento automático, menções `@`, exibição de tooltips e acionamento da sidebar.
...
- [x] 1. Gerar a feature `src/features/editor` (`npm run generate:feature -- --name="editor"`).
- [x] 2. Implementar a estrutura de estado e autosave em `src/features/editor/model/` e `hooks/useAutoSave.ts`.
- [x] 3. Construir o componente `RichEditor` com suporte a marcação semântica de lore e atalhos de teclado.
- [x] 4. Integrar o motor `loreMatcher` da feature `lore` para destacar termos dinamicamente no conteúdo.
- [x] 5. Implementar o componente `MentionMenu` para inserção com `@`.
- [x] 6. Integrar a sidebar retrátil `EditorLoreSidebar` permitindo consultar e editar fichas diretamente na tela de escrita.
- [x] 7. Criar a página `EditorPage` e registrar a rota `/books/:bookId/editor/:chapterId`.
- [x] 8. Escrever testes automatizados em `src/features/editor/tests/`.
